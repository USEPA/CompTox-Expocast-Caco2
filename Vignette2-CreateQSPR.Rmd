...
This vignette creates a quantitative structure-property relationship (QSPR)
model form apical:basal membrane permeability. The method of random forests is
applied to a set of measurement data both from literature and newly generated
for this work.
...
## Building a QSPR for Membrane Permeability
...
save(chembl,file=paste0(loc.wd,"/r_data/QSPR/chembl_caco2.RData")) 
```
...
```{r qspr_compiletrainingset}
  ...
  caco2.dt2 <- as.data.table(read_excel(paste0(loc.wd, "/results_for_paper/caco2_data.xlsx")))
  dim(caco2.dt2)
...
# Pull from ChEMBL by Risa:
  load(paste0(loc.wd,"/r_data/QSPR/chembl_caco2.RData"))
...
#                   alpha=0.2, position="identity")+
  ...
  #                    ymin = 130,ymax = 130,xmax = -3, xmin = -3)
    ...
    ggsave(FigPabSourceHist, file = paste0(loc.wd, "/results_for_paper/Fig_QSPR_TrainSet_histogram.tiff"),
...
```{r qspr_writeoutdtxsidsforCCD}
  write.table(subset(train.set,regexpr("DTXSID",dtxsid)!=-1)$dtxsid, 
                     file=paste0(loc.wd,"/r_data/QSPR/DataDTXIDsforDashboard.txt"), 
               row.names=FALSE,
               col.names=FALSE,
               quote=FALSE)
```
...
```{r qspr_mergesmiles}
  CCD <- as.data.frame(
    read_excel(paste0(loc.wd,"/r_data/QSPR/smilesfromCCD.xlsx"),sheet=2))
  dim(CCD)
...
length(unique(smiles$dtxsid))
  write.table(smiles,
            ...
            quote = FALSE,
            file=paste0(loc.wd,"/r_data/QSPR/trainset.smi"))
```
...
# Put all measurements of test chems into the test bin:
  ...
  # Drop equal weight bins (useful for dividing data, not needed for model 
  #  building):
  all.desc <- all.desc[, !(colnames(all.desc) %in% c("Pab2Bin",
                                                     "Pab3Bin",
                                                     "Pab4Bin",
                                                     "Pab5Bin"))]  
  dim(all.desc)
  ...
  save(all.desc, file=paste0(loc.wd, "/r_data/QSPR/all.desc.RData"))
  save(test.set,file=paste0(loc.wd, "/r_data/QSPR/testset.RData"))
  save(train.set,file=paste0(loc.wd, "/r_data/QSPR/trainset.RData"))
  write.table(train.set,sep="\t",row.names=FALSE,
            file=paste0(loc.wd,"/r_data/QSPR/trainset.txt"))
...
# indicating which:
  ...
  save(all.caco2,file=paste0(loc.wd,"/r_data/QSPR/alldata.RData"))
  write.table(subset(all.caco2,!is.na(dtxsid)), 
              file=paste0(loc.wd,
                "/results_for_paper/TableAllCaco2PabData_10e-6cmps.txt"), 
              row.names=FALSE,
              sep="\t")
# Fraction abouve Pab > 5
dim(subset(all.caco2,Pab>5))[1]/dim(all.caco2)
# Fraction abouve Pab > 1
dim(subset(all.caco2,Pab>1))[1]/dim(all.caco2) 
```

```{r qspr_resultsforpaper}
load(file=paste0(loc.wd,"/r_data/QSPR/testset.RData"))
load(file=paste0(loc.wd,"/r_data/QSPR/trainset.RData"))
...
### Clean out low variance variables
...
save(train_msd, file=paste0(loc.wd,"/r_data/QSPR/trainingdata-meansd.RData"))
```
...
#the correlation with HLH. This ends up with slightly more predictors 
  ...
  save(reduced.desc,file=paste0(loc.wd,"/r_data/QSPR/trainingdata-reduced.RData"))
```

### Configure caret for 5-fold cross validation
```{r qspr_tenfoldCV}
...
save(classmod, train.set, test.set, reduced.desc, file=paste0(loc.wd,"/r_data/QSPR/classmodels.RData"))
```

```{r qspr_regmod_train}
set.seed(seed)
regmod <- list()
for (this.model in c("RegMod"))
{
  print(this.model)
  out <- tuneRF(x=reduced.desc, 
                y=log10(train.set[,"Pab"]),
                mtryStart=floor(dim(reduced.desc)[2]^(1/2)), #j is sizes 
                ntreeTry=500,
                stepFactor=1.25,
                improve=0.005,
  ...
  tunegrid <- expand.grid(.mtry=bestmtry)
  set.seed(seed)
  regmod[[this.model]] <-train(x=reduced.desc,
                   y=log10(train.set[,"Pab"]), 
                   method="rf", 
                   metric="RMSE", 
                   trControl=control, 
  ...
  regmod[[this.model]]$bestTune
  regmod[[this.model]]$finalModel
  regmod[[this.model]]$trainingData
  regmod[[this.model]]$results # the average across all cross-validations and reps
  hist(regmod[[this.model]]$resample$RMSE)
  hist(regmod[[this.model]]$resample$Rsquared)
}
save(regmod, train.set, test.set, reduced.desc, file=paste0(loc.wd,"/r_data/QSPR/regmodel.RData"))
```
...
load(file=paste0(loc.wd,"/r_data/QSPR/classmodels.RData"))
load(file=paste0(loc.wd,"/r_data/QSPR/regmodel.RData"))
...
for (this.model in names(regmod))
{
  regmod[[this.model]]$testsetpred <- predict(regmod[[this.model]],
                                                test.set[,colnames(reduced.desc)])
  regmod[[this.model]]$pred.var <- signif((log10(test.set[,"Pab"])-
    regmod[[this.model]]$testsetpred)^2, 4)
  regmod[[this.model]]$tot.var <- signif((log10(test.set[,"Pab"])-
    mean(log10(test.set[,"Pab"])))^2, 4)
  regmod[[this.model]]$RMSE <- signif((
    mean(regmod[[this.model]]$pred.var))^(1/2),4)
  regmod[[this.model]]$rsquared <- signif(1 -
    sum(regmod[[this.model]]$pred.var) /
    sum(regmod[[this.model]]$tot.var), 4)
  regmod[[this.model]]$testsetbinpred <- 
    rep(2,length(regmod[[this.model]]$testsetpred))
  regmod[[this.model]]$testsetbinpred[
    10^(regmod[[this.model]]$testsetpred) > 
      min(subset(train.set,LogPab3Bin==3)$Pab)] <- 3
  regmod[[this.model]]$testsetbinpred[
    10^(regmod[[this.model]]$testsetpred) < 
      max(subset(train.set,LogPab3Bin==1)$Pab)] <- 1
  regmod[[this.model]]$testsetbinpred <- as.factor(
    regmod[[this.model]]$testsetbinpred)
  regmod[[this.model]]$confusion <- confusionMatrix(
    data = regmod[[this.model]]$testsetbinpred, 
    reference = test.set[,"LogPab3Bin"])
  }
```
...
# Keep only the models where clustering was used for binning:
qspr.summary <- qspr.summary[,regexpr("LogPab",colnames(qspr.summary))!=-1]
for (this.model in colnames(qspr.summary))
{
  for (this.bin in sort(unique(train.set[,this.model])))
  {
    qspr.summary[paste0("Bin",this.bin),this.model] <-
      paste0(signif(median(
               train.set[train.set[,this.model]==this.bin,"Pab"]),
               SIGFIGS),
             " (",
             signif(quantile(
               test.set[classmod[[this.model]]$testsetpred==this.bin,"Pab"], 0.025),
               SIGFIGS),
             " - ",
             signif(quantile(
               test.set[classmod[[this.model]]$testsetpred==this.bin,"Pab"], 0.975),
               SIGFIGS),
             ")")
  }
}
regmod.results <- qspr.summary
regmod.results[,1]<-NA
regmod.results["Accuracy",1] <- 
  signif(regmod[[1]]$confusion$overall["Accuracy"], 3)
regmod.results["Kappa",1] <- 
  signif(regmod[[1]]$confusion$overall["Kappa"], 3)
colnames(regmod.results)[1] <- "Continuous"
qspr.summary <- cbind(regmod.results[,"Continuous"],qspr.summary)
colnames(qspr.summary)[1] <- "Continuous"
save(qspr.summary,file=paste0(loc.wd,"/r_data/QSPR/qsprbinnumbersummary.RData"))
```

```{r qspr_resultstable}
load(paste0(loc.wd,"/r_data/QSPR/qsprbinnumbersummary.RData"))
knitr::kable(qspr.summary, 
             caption = "Summary of test set performance of RF models as a function of how Caco2 data are subdivided into categories. Row's labeled \"Bin[Number]\" give the median of the training data bin while the 95% interval is calculated from the test set measurments withing each bin.",
             floating.environment="sidewaystable") 
write.csv(qspr.summary,
          na="",
          file=paste0(loc.wd,"/results_for_paper/Tableqsprbinnumbersummary.csv"))
```
...
save(rfeClass, train.set, reduced.desc, chosen.model,
     file=paste0(loc.wd,"/r_data/QSPR/recursive_feature_elimination.RData"))
save(chosen.model,
     file=paste0(loc.wd,"/r_data/QSPR/chosen_model.RData"))
```

### Regression Model

We have unevenly distributed training data -- most training examples are 
well-absorbed. The random forest algorithm used does not allow for 
weighting data. By binning the results we decrease the weight on data in the
well-absorbed bin and increase the weight on the outlier bins. This produces a
model that is slightly wore for the most common cases, but is potentially
better at predicting outliers. To demonstrate this we build an optimized
regression model weighting all data equally. Potentially, we could also explore 
replicating the data so that outliers occur more often. Given the size of the
training set, I feel that categorical models more accurately represent the
actual confidence based on the avaialble data.
...
subsets <- c(seq(2,8,2),seq(10,35,5))
set.seed(12345)
rfeReg <- suppressMessages(rfe(x=reduced.desc,
                y=log10(train.set[,"Pab"]),
                rfeControl = ctrl,
                sizes=subsets,
                metric="Accuracy",
...
save(rfeReg, train.set, reduced.desc,
     file=paste0(loc.wd,"/r_data/QSPR/recursive_feature_elimination_reg.RData"))
```
...
#### Classification Model
```{r qspr_parsimony_class}
load(paste0(loc.wd,"/r_data/QSPR/recursive_feature_elimination.RData"))
...
FigRFE <- ggplot(rfeClass$results) + 
  ...
  geom_vline(xintercept=num.opt.desc, color="blue",linetype="dashed")
ggsave(FigRFE, 
       file = paste0(loc.wd, "/results_for_paper/Fig_QSPR_RFE_Class.tiff"),
...
save(opt.desc,num.opt.desc, 
     file=paste0(loc.wd,"/r_data/QSPR/optimal_parameters.RData"))
```

#### Regression Model
```{r qspr_parsimony_reg}
load(paste0(loc.wd,"/r_data/QSPR/recursive_feature_elimination_reg.RData"))

ggplot(rfeReg$results) + 
  geom_line(aes(x=Variables, y=RMSE)) +
  xlab("Number of Descriptors") + 
  ylab("Regression Model RMSE")#+
 # geom_vline(xintercept=num.opt.desc, color="blue",linetype="dashed")
```
...
generally more parsimonious).
```{r qspr_manualrfesize}
num.opt.desc.reg <- 25 # JFW, June 2023

FigRFE.Reg <- ggplot(rfeReg$results) + 
  geom_line(aes(x=Variables, y=RMSE)) +
  xlab("Number of Descriptors") + 
  ylab("Model RMSE") +
  geom_vline(xintercept=num.opt.desc, color="blue",linetype="dashed")
ggsave(FigRFE.Reg, 
       file = paste0(loc.wd, "/results_for_paper/Fig_QSPR_RFE_Reg.tiff"),
       height = 3, width = 3, dpi = 600, compression = "lzw")
print(FigRFE.Reg)

desc.set <- subset(rfeReg$variables,Variables==num.opt.desc)
#use aggregate to calculate mean ranking score (under column "Overall")
desc.set <- aggregate(desc.set[, c("Overall")], list(desc.set$var), mean)
...
save(opt.desc,num.opt.desc, 
     file=paste0(loc.wd,"/r_data/QSPR/optimal_parameters_reg.RData"))
```

### Build optimal QSPR model using parsimonious descriptors

#### Classification model
```{r qspr_buildoptmodel}
load(file=paste0(loc.wd,"/r_data/QSPR/optimal_parameters.RData"))
...
save(classmodopt, train.set, reduced.desc, opt.desc, chosen.model,
     file=paste0(loc.wd,"/r_data/QSPR/optimal_model.RData"))
```

#### Regression model
```{r qspr_buildoptmodel_reg}
load(file=paste0(loc.wd,"/r_data/QSPR/optimal_parameters_reg.RData"))

metric <- "RMSE"
set.seed(seed)
out.reg <- suppressMessages(tuneRF(x=reduced.desc[,opt.desc], 
              y=log10(train.set[,"Pab"]),
              mtryStart=floor(num.opt.desc^(1/2)), #j is sizes 
              ntreeTry=1000,
              stepFactor=1.25,
              ...
              plot=TRUE, 
              dBest=FALSE))
bestmtry <- out.reg[which(out.reg[,2]==min(out.reg[,2])),1]
if (length(bestmtry)>1)
{
  bestmtry= max(bestmtry)
}
tunegrid <- expand.grid(.mtry=bestmtry)
set.seed(12345)
regmodopt <- suppressMessages(train(
  x=reduced.desc[,opt.desc],
  y=log10(train.set[,"Pab"]), 
  method="rf",  
  metric=metric, 
  trControl=control, 
  tuneGrid = tunegrid))
save(regmodopt, train.set, reduced.desc, opt.desc,
     file=paste0(loc.wd,"/r_data/QSPR/optimal_model_reg.RData"))
```
...
tiff(file = paste0(loc.wd, "/results_for_paper/Fig_QSPR_VarImportance.tiff"),
...
save(opt.testsetpred, opt.confusion, opt.testsummary, test.set, chosen.model,
     file=paste0(loc.wd,"/r_data/QSPR/optimal_model_test_set_performance.RData"))                          
```
...
```{r caco2_qspr_pab_eval_testonly_fig}
  ...
  print(FigQSPRPredPabTest)
    ggsave(FigQSPRPredPabTest, 
           file = paste0(loc.wd, 
                         "/results_for_paper/Fig_QSPRPredPabTestOnly.tiff"),
         height = 3, width = 3, dpi = 600, compression = "lzw")
```

#### Regression model
```{r qspr_testset2_reg}
load(file=paste0(loc.wd,"/r_data/QSPR/optimal_model_reg.RData"))
load(file=paste0(loc.wd,"/r_data/QSPR/testset.RData"))
load(file=paste0(loc.wd,"/r_data/QSPR/trainingdata-meansd.RData"))
load(file=paste0(loc.wd,"/r_data/QSPR/qsprbinnumbersummary.RData"))
...
opt.testsetpred.reg <- 10^predict(regmodopt,
                       test.set[,opt.desc])
test.set$Pab.Pred <- opt.testsetpred.reg 
# Convert regressions predictions to bins:
bin1max <- max(subset(test.set,LogPab3Bin=="1")$Pab)
bin2max <- max(subset(test.set,LogPab3Bin=="2")$Pab)
test.set$Pab.RegPredBin <- 3
test.set[test.set$Pab.Pred < bin2max,"Pab.RegPredBin"] <- 2
test.set[test.set$Pab.Pred < bin1max,"Pab.RegPredBin"] <- 1
test.set$Pab.RegPredBin <- as.factor(test.set$Pab.RegPredBin )
opt.testsetpred.reg <- test.set$Pab.RegPredBin
```
...
```{r caco2_qspr_pab_eval_testonly_fig_reg}
      FigQSPRPredPabTest.Reg <- ggplot(test.set)+
      geom_point(aes(x = Pab, 
                       y = Pab.Pred, 
                       color=Data.Origin, 
                       shape=Data.Origin), 
       ...
       # lims(x = c(-5,3), y = c(-5,3)) +
        ...
        scale_colour_viridis_d()+
        gtheme
  print(FigQSPRPredPabTest.Reg)
    ggsave(FigQSPRPredPabTest.Reg, 
           file = paste0(loc.wd, 
                         "/results_for_paper/Fig_QSPRPredPabTestOnly_Reg.tiff"),
         height = 3, width = 3, dpi = 600, compression = "lzw")
```
