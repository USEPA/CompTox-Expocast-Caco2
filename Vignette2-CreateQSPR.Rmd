...
exposure estimates, and the influence of different extrapolation assumptions on 
the bioactivity-to-exposure ratio (BER) was evaluated. Generally, the large 
...
```{r qspr_compiletrainingset}
  # Load data
  load(paste0(loc.wd,"/r_data/processed/our_caco2_25MAR2019.RData"))
...
```{r qspr_maketestset}
  ...
  for (this.bin in 1:5)
  {
    this.bin.rows <- which(all.desc$Pab5Bin == this.bin)
    test.rows <- sample(this.bin.rows,
                        round(TEST.FRAC*length(this.bin.rows)))
    all.desc[test.rows,"Test"] <- 1
  }
  dim(all.desc)
  ...
  save(all.caco2,file=paste0(loc.wd,"/r_data/alldata.RData"))
    write.table(subset(all.caco2,!is.na(dtxsid)), 
                     file=paste0(loc.wd,"/r_data/AllCaco2PabData_10e-6cmps.txt"), 
              row.names=FALSE,
              sep="\t")
```
```{r qspr_resultsforpaper}
print(paste0(dim(test.set)[1]," chemicals in test set."))
print(paste0(dim(subset(test.set,Data.Origin=="EPA"))[1]," EPA chemicals in test set."))
print(paste0(dim(subset(test.set,Data.Origin=="Lanevskij"))[1]," Lanevskij chemicals in test set."))
print(paste0(dim(subset(test.set,Data.Origin=="Obringer"))[1]," Obringer chemicals in test set."))
print(paste0(dim(subset(test.set,regexpr("ChEMBL",Data.Origin)!=-1))[1]," ChEMBL chemicals in test set."))

print(paste0(dim(train.set)[1]," chemicals in training set."))
print(paste0(dim(subset(train.set,Data.Origin=="EPA"))[1]," EPA chemicals in training set."))
print(paste0(dim(subset(train.set,Data.Origin=="Lanevskij"))[1]," Lanevskij chemicals in training set."))
print(paste0(dim(subset(train.set,Data.Origin=="Obringer"))[1]," Obringer chemicals in training set."))
print(paste0(dim(subset(train.set,regexpr("ChEMBL",Data.Origin)!=-1))[1]," ChEMBL chemicals in training set."))
```
...
write.csv(qspr.summary,file=paste0(loc.wd,"/r_data/qsprbinnumbersummary.csv"))
```
...
save(httk.caco2.qspr,
     file=paste0(loc.wd,"/r_data/httk_qspr_preds.RData")) 
```
