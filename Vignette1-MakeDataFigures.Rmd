...
# Caco2.Pab <= 0.55 10^-6 cm / s needed for Human Peff <= 0.53
Fabs(Peff=Peff(Caco2.Pab=0.028), factor=rat.fabs.factor)
Caco2.Pab.thresh.rat <- 0.028
cat(paste0("For rats the equations predict that we need to observe PappAB <= ",
           Caco2.Pab.thresh.rat,
           " 10^-6 cm /s to predict Fabs <= 0.5."))

cat(paste0("Only ",
           sum(subset(caco2.perchem,!is.na(mPab))$mPab < Caco2.Pab.thresh.rat),
           " chemicals (",
           100*signif(sum(subset(caco2.perchem,!is.na(mPab))$mPab < 
                            Caco2.Pab.thresh.rat) /
                        dim(subset(caco2.perchem,!is.na(mPab)))[1],2),
           "%) have measured Pappab indicating Fabs < 0.5 for rat."))
```
