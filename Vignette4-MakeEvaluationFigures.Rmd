...
```{r Caco_permeability_calcs} 
  load(file=paste0(loc.wd, "/r_data/processed/caco2-invivo-compare1.RData")) 
   ...
   # Yu and Amidon (1999) Equation 11:
        .[,fah_darwichh:=1 - (1 + 2 * peffh * MRTh / 7 / Rsih * 60 / 10^4)^-7] %>% 
        .[,fah_darwichr:=1 - (1 + 2 * peffr * MRTr / 7 / Rsir * 60 / 10^4)^-7] %>% 
   # Yu and Amidon (1999) Equation 11:
...
```{r adjusted.rsquared.resample}
resample.adj.rsquared <- function(obs, pred, p=1)
{
  N <- length(obs)
  
  new.samp<- sample(1:N, N, replace=TRUE)
  new.obs <- obs[new.samp]
  new.pred <- pred[new.samp]
  
  SS.tot<- sum((new.obs-mean(new.obs))^2)
  SS.res <- sum((new.obs-new.pred)^2)
...
# Restrict to only non-NA values:
  ...
  for (i in 1:10000)
  {
    ...
    RMSE.samples[i] <- sqrt(mean(this.obs - this.pred)^2)
  }
  eval.table[this.evaluation,c("N.Chem","Reference","R2","R2.Range","RMSE")] <- c(
    num.chem,
