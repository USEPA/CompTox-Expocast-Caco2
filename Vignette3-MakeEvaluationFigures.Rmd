...
```{r make_CacoFabsRat_fig}
  ...
  FigCacoPredFbioRat <- ggplot(invivo.table)+
        geom_jitter(aes(y = invivo_fbioh, x = pk_fbior, color=Class, shape=Class), 
                   alpha = 0.55,
                   size=3,
  ...
  ggsave(FigCacoPredFbioRat, file = paste0(loc.wd, "/results_for_paper/Fig_CacoPredFbioRat.tiff"),
         height = 3, width = 3, dpi = 600, compression = "lzw")
```
...
```{r make_prediction_histograms2}
 ...
 #   scale_x_log10(labels=scientific_10)+
# First bin from QSPR:
    geom_vline(xintercept =  calc_fabs.oral(Params=list(Caco2.Pab=0.23)), 
      linetype="dashed", 
      color = "blue", size=1)+
# Second bin from QSPR:
    geom_vline(xintercept =  calc_fabs.oral(Params=list(Caco2.Pab=2.08)), 
      linetype="dashed", 
      color = "blue", size=1)+
# Third bin from QSPR:
    geom_vline(xintercept =  calc_fabs.oral(Params=list(Caco2.Pab=20)), 
      linetype="dashed", 
      color = "blue", size=1)+
    theme_bw()+
        scale_fill_viridis(discrete=2)+
